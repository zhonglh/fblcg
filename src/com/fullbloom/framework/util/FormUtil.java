...
public class FormUtil {
	...
	private static String GetRadios(JSONObject item, Map<String, Object> formData, String action) {
		...
		for (int f = 0; f < radiosOptions.size(); f++) {
			...
			if (value == null) {
				String check = (json.has("checked") && json.getString("checked") != null)
						? json.getString("checked").toString() : "";
	...
	private static String GetSelect(JSONObject item, Map<String, Object> formData, String action) {
		...
		if ("view".equals(action)) {// 查看
			return MessageFormat.format(temp_view, "&nbsp;",
					value != null ? value : item.getString("value").toString().split(",")[0]);
		} else
			return temp_html;
	}
	...
	private static String GetMacros(JSONObject item, Map<String, Object> formData, String action) {
		...
		if ("view".equals(action))
			return value.replace("${userId}", ClientManager.getInstance().getClient().getTsUser().getId())
					.replace("${userName}", ClientManager.getInstance().getClient().getTsUser().getUserName());
		if (value != null) {
		...
		for (int row = 0; row < rowCount; row++) {
			...
			for (int i = 0; i < tdCount; i++) {
				...
				if (i == tdCount - 1)// 最后一列不显示
				{
					...
					// tr +=
					// string.Format("<td><a href=\"javascript:void(0);\"
					// class=\"delrow {0}\">删除</a></td>",
					// dataValue != null ? "" : "hide");
				} else {
					if ("view".equals(action)) {
						...
						if ("text".equals(type))
							...
							tr += MessageFormat.format(
									"<td><textarea class=\"input-medium\" name=\"{0}\" >{1}</textarea></td>", tdname,
									tdValue, sum);
