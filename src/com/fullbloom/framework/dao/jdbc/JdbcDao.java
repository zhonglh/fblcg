package com.fullbloom.framework.dao.jdbc;
...
//	public static final String DB2_SQL = "select inner2_.*,  rownumber() over( order by  order of inner2_) as rownumber_ from  ({0})   fetch first {1} rows only ) as inner2_ ) as inner1_         where       rownumber_ > {2}       order by     rownumber_";	//db2
	public static final String DB2_SQL = "select * FROM   ( SELECT B.*, ROWNUMBER() OVER() AS RN FROM   ({0}) AS B   )AS A WHERE A.RN BETWEEN {2} AND  {1}  "; //db2
//	public static final String DB2_SQL = "select * FROM   ( SELECT B.*, ROWNUMBER() OVER() AS RN FROM   ({0}) AS B   )AS A WHERE A.RN > {2} AND A.RN <= {1}  "; //db2
	@Autowired
	...
	public static String jeecgCreatePageSql(String sql, int page, int rows){
		...
		if(ResourceUtil.getJdbcUrl().indexOf(DATABSE_TYPE_MYSQL)!=-1){
			...
			}else if(ResourceUtil.getJdbcUrl().indexOf(DATABSE_TYPE_DB2)!=-1){
				sqlParam[2] = Integer.toString(beginIndex+1);
				sql = MessageFormat.format(DB2_SQL, sqlParam);
			}
		}
		return sql;
	}
